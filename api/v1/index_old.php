...
// User cek Login
$app->post('/user/smfLogin', function() use ($app) {
    // check for required params
    // verifyRequiredParams(array('name', 'password'));

    // reading post params
    $nama     =  $app->request->post('name');
    $passwords = $app->request->post('password');

    // validating email address
    // validateEmail($email);
    $password=md5($passwords);
    $db = new DbHandler();
    $response = $db->getUsernPasswd($nama,$password);
    // echo json response
    echoRespnse(200, $response);
});

$app->put('/user/token', function() use ($app) {
    ...
    $db =new DbHandler();
    $response = $db->saveToken($uid,$token);
    // echo json response
    echoRespnse(200, $response); 
});    
